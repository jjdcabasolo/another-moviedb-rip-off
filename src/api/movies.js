...
export const getPopularMovies = (api_key, success, fail, after = () => {}) =>
  axios
...
export const getNowPlayingMovies = (api_key, success, fail, after = () => {}) =>
  axios
...
export const getTopRatedMovies = (api_key, success, fail, after = () => {}) =>
  axios
...
export const getUpcomingMovies = (api_key, success, fail, after = () => {}) =>
  axios
...
export const getTrendingMovies = (api_key, success, fail, after = () => {}) =>
  axios
    .get("/trending/movie/day", { params: { api_key } })
    .then((response) => success(response))
    .catch((error) => fail(error))
    .finally(() => after());
  ...
  axios
    .get(`/movie/${movie_id}`, {
      params: {
        api_key,
        append_to_response:
          "videos,credits,external_ids,recommendations,reviews",
      },
    })
    .then((response) => {
      const { data } = response;
      ...
      if (collection) {
        getMovieCollection(
          ...
          (collectionResponse) => {
            ...
            collectionData.parts = parts.sort((a, b) =>
              moment.utc(a.release_date).diff(moment.utc(b.release_date))
            );
...
export const searchMovie = (api_key, query, success, fail, after) =>
  axios
    .get(`/search/movie`, { params: { api_key, query } })
